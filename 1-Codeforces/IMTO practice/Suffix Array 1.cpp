...
int ubd(string &q, int &m, string &s, int &n) {
	int l = 0, r = p.size() - 1, res = 0;
	while (l <= r) {
		int mid = (l + r) >> 1, pos = p[mid];

		string temp = s.substr(pos, min(m, n - pos));
		if (temp <= q) l = res = mid + 1;
		else r = mid - 1;
	}
...
int lbd(string &q, int m, string &s, int& n) {
	int l = 0, r = p.size() - 1, res = 0;
	while (l <= r) {
		int mid = (l + r) >> 1, pos = p[mid];
...
signed main() {
	...
	ll cnt = 0;
	for (int i = 0; i < n; ++i) cnt += (n - p[i] - 1) - lcp[i];
	cout << cnt << endl;
}
